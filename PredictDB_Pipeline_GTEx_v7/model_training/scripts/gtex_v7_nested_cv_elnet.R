...
get_filtered_snp_annot <- function(snp_annot_file_name) {
  snp_annot <- read.table(snp_annot_file_name, header = T, stringsAsFactors = F, fill = T) %>%
  ...
  print(paste0("snp annotation dimension before filtering: ", dim(snp_annot)[1]))
  snp_annot
}


get_maf_filtered_genotype <- function(genotype_file_name,  maf, samples) {
  gt_df <- read.table(genotype_file_name, header = T, stringsAsFactors = F, row.names = 1, check.names = F)
  print(paste0("genotype file dimension before filtering: ", dim(gt_df)[1]))
  ...
  #print(paste0("genotype file dimension after filtering: ", dim(gt_df)[2]))      
  gt_df
}


get_final_genotype <- function(gt_df, snp_annot){
  length(intersect(snp_annot$varID, colnames(gt_df)))
}
...
get_gene_annotation <- function(gene_annot_file_name, chrom, gene_types=c('protein_coding', 'pseudogene', 'lincRNA')){
  ...
  print(paste0("gene annotation file dimension after filtering: ", dim(gene_df)[1]))  
  gene_df
}
...
get_gene_expression <- function(gene_expression_file_name, gene_annot) {
  ...
  print(paste0("expression file dimension after filtering: ", dim(expr_df)[2]))
  expr_df
}
...
get_cis_genotype <- function(gt_df, snp_annot, coords, cis_window) {
  ...
  cis_gt <- gt_df[, intersect(snp_info$varID, colnames(gt_df))] #this is faster than previous method
...
get_covariates <- function(covariate_file_name, samples) {
  cov_df <- t(read.table(covariate_file_name, header = TRUE, stringsAsFactors = FALSE, row.names = 1))
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
  ...
  print(paste0("genotype file dimension after filtering: ", get_final_genotype(gt_df, snp_annot)))
