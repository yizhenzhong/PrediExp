    ...
    def __call__(self, rsid):
        if self.tuples:
            ...
            for tup in self.db.query("SELECT gene, weight, eff_allele FROM weights WHERE rsid=?", (rsid,)):
                yield tup
    def get_predictors(self, gene):
            return [tup[0] for tup in self.db.query('SELECT "n.snps.in.model" from extra where gene=?', (gene,))][0]
...
class TranscriptionMatrix:
    ...
    def fill(self, counts, gene):
            self.predictors[self.gene_index[gene],1]=counts
    ...
    def write(self, predictor_file):
        np.savetxt( predictor_file, self.predictors,delimiter='\t', header='used\ttotal',fmt='%d')
...
def main():
    ...
    OUT_PREDI_NAME = args.output_prefix + "_predictors.txt" if args.output_prefix else "predictors.txt"
    ...
    if PREDICT:
        ...
        for rsid, allele, dosage_row in get_all_dosages(DOSAGE_DIR, DOSAGE_PREFIX, DOSAGE_BUFFER):#allele is the effect allele in dosage
            for gene, weight, ref_allele in get_applications_of(rsid):  #effect allele in weights/
               transcription_matrix.update(gene, weight, ref_allele, allele, dosage_row)
               counts = get_applications_of.get_predictors(gene)
               transcription_matrix.fill(counts, gene)

            
        transcription_matrix.save(PRED_EXP_FILE)
        print(OUT_PREDI_NAME)
        transcription_matrix.write(OUT_PREDI_NAME)
    if ASSOC:
