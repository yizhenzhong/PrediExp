...
tiss <- "AA_hepatocyte_RPKM"
driver <- dbDriver('SQLite')
# Extra table ----
model_summaries <- read.table('../summary/nested_cv_rpkm_chr1_model_summaries.txt', header = T, stringsAsFactors = F, fill = T)
tiss_summary <- read.table('../summary/nested_cv_rpkm_chr1_tiss_chr_summary.txt', header = T, stringsAsFactors = F, fill = T)


for (i in 2:22) {
  model_summaries <- rbind(model_summaries,
                           read.table('../summary/nested_cv_rpkm_chr' %&% as.character(i) %&% '_model_summaries.txt', header = T, stringsAsFactors = F, fill = T))
  tiss_summary <- rbind(tiss_summary,
                        read.table('../summary/nested_cv_rpkm_chr' %&% as.character(i) %&% '_tiss_chr_summary.txt', header = T, stringsAsFactors = F, fill = T))
}
#print(model_summaries[1:3,1:3])

model_summaries <- dplyr::rename(model_summaries, gene = gene_id)
write.table(model_summaries, "../dbs/model_summaries_rpkm.txt", quote =  F, row.names = T, col.names = T)
write.table(tiss_summary, "../dbs/tissue_summaries_rpkm.txt", quote =  F, row.names = T, col.names = T)
print(dim(model_summaries))

print(model_summaries[1:3,1:3])
conn <- dbConnect(drv = driver, '../dbs/AA_hepatocyte_rpkm.db')
...
# Weights Table -----
weights <- read.table('../weights/nested_cv_rpkm_chr1_weights.txt', header = T, stringsAsFactors = F, fill = T)
for (i in 2:22) {
  weights <- rbind(weights,
                   read.table('../weights/nested_cv_rpkm_chr' %&% as.character(i) %&% '_weights.txt', header = T, stringsAsFactors = F, fill = T))
}
